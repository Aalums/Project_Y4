...
class SuperLearnerClassifier(BaseEstimator, ClassifierMixin):
    ...
    ori_input_2train_meta: boolean , optional (default = False)
        A boolean flag variable to specify whether the meta learner should be trained on the trainin set with original features or not.
            False => trained only on 'output' from base estimators
            True => train on 'original features/inputs' plus 'output' from base estimators

    meta_model_type: string, optional (default = "LR")
        An option in string, only either "DCT" or "LR" or "SVM", to specify the type of model to use at the stack layer for a meta learner
    ...
    def __getattr__(self, key):
        if key.startswith('__') and key.endswith('__'):
            return super(SuperLearnerClassifier, self).__getattr__(key)
        return self.__getitem__(key)

    def __init__(self,
                 n_classes,
                 estimators=[DecisionTreeClassifier(criterion="entropy", max_depth=10),
                             ExtraTreeClassifier(),
                             LogisticRegression(),
                 ...
                 cv_folds=10,
                 # To avoid overfiiting of the stacked level classifier, specify the number of K for k-fold cross validation in level-one base classifiers
                 proba_2train_meta=False,
                 # Specify whether the stacked layer classifier should be trained on probability output from the base classifiers or not; if not, trained on label outputs
                 ori_input_2train_meta=False,
                 # Specify whether the original input is added, together with the output from the base classifiers, to train the stacked layer classifier
                 meta_model_type="LR",
                 # Specify the model type at the stacked layer either Decision Tree or Logistic Regression, meta_model_type = {"DCT", "LR"}
                 est_accuracy=False,
                 est_corr=False,
                 debug_mode=False):
        ...
        if not isinstance(n_classes, int):
            raise TypeError(
                "Please set the \'n_classes\' in integer, to specify the number of target classes for prediction.")
        if not (
                        meta_model_type == "DCT" or meta_model_type == "LR" or meta_model_type == "SVM" or meta_model_type == "RF"):
            raise ValueError(
                "In the current support of a meta learner, \'meta_model_type\' must be only \'DCT\' for DecisionTreeClassifier or \'LR\' for LogisticRegression or \'SVM\' for SupportVectorMachine.")
        if len(estimators) < 5 or len(estimators) > 10:
            raise ValueError(
                "It is sensible to use a set of 5-10 algorithms as base learners. The current number of base estimators is: " + str(
                    len(estimators)))
    ...
    def fit(self, X, y):
        ...
        if self.meta_model_type == "DCT":
            self.meta_learner = DecisionTreeClassifier(criterion="entropy", max_depth=10)
        elif self.meta_model_type == "SVM":
            self.meta_learner = SVC(C=0.1, kernel='linear', probability=True)
        elif self.meta_model_type == "LR":
            self.meta_learner = LogisticRegression()
        elif self.meta_model_type == "RF":
            self.meta_learner = RandomForestClassifier(max_depth=15, n_estimators=200)
        ...
        if isinstance(self.meta_learner, DecisionTreeClassifier):
            self._print_debug("--Meta learner: DecisionTreeClassifier")
        elif isinstance(self.meta_learner, LogisticRegression):
            self._print_debug("--Meta learner: Logistic Regression")
        elif isinstance(self.meta_learner, SVC):
            self._print_debug("--Meta learner: SVM")
        elif isinstance(self.meta_learner, RandomForestClassifier):
            self._print_debug("--Meta learner: RF")
